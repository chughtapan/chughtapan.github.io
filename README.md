...
See the [changelog](CHANGELOG.md) for this site's release history.
...
Finally, build the local image and run your website:

```bash
$ docker compose up --build
```

The first build downloads the Ruby base image and installs the project's locked dependencies, so it can take a few minutes.

Now, feel free to customize the theme however you like (don't forget to change the name!). After you are done, use the same command (`docker compose up --build`) to render the webpage with all your changes. Also, make sure to commit your final changes.

> To change port number, you can edit `docker-compose.yml` file.

<details><summary>(click to expand) <strong>Use the legacy local Compose filename:</strong></summary>

> `docker-local.yml` is retained for compatibility and uses the same local build flow.

Build and run a new docker image using:
```bash
$ docker compose -f docker-local.yml up --build
```
> Re-run this command after updating Jekyll or other dependencies so Docker rebuilds the image.
...
Use Ruby 3.4.10 (recorded in `.ruby-version`) and Bundler 4.0.15 (recorded in `Gemfile.lock`). For ease of managing Ruby versions, consider using [rbenv](https://github.com/rbenv/rbenv). Then click [Use this template](https://docs.github.com/repositories/creating-and-managing-repositories/creating-a-repository-from-a-template) above the file list, create a new repository at `github.com:<your-username>/<your-repo-name>` from `github.com:alshedivat/al-folio`, and run:
  ...
  repo-sync:
    ...
    steps:
      ...
      - name: actions-template-sync
        uses: AndreasAugustin/actions-template-sync@v2.5.3
        with:
...
**al-folio** supports optional math typesetting through [MathJax](https://www.mathjax.org/) and code syntax highlighting using [GitHub style](https://github.com/jwarby/jekyll-pygments-themes). MathJax is disabled by default on this site because its current content does not use math; set `enable_math: true` in `_config.yml` to enable it:
